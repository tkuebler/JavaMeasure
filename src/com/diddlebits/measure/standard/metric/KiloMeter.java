/*
 * Meter.java
 *
 * Created on October 3, 2003, 10:13 PM
 */

package com.diddlebits.util.measure.standard.metric;

import com.diddlebits.util.measure.standard.UnitOfMeasure;
import com.diddlebits.util.measure.standard.StandardOfMeasure;
import com.diddlebits.util.measure.quantity.Quantity;
import com.diddlebits.util.measure.standard.Metric;
import com.diddlebits.util.measure.quantity.Length;

/**
 *
 * @author  tkuebler
 */
public class KiloMeter implements UnitOfMeasure {
    
    private static final StandardOfMeasure STANDARD = new Metric();
    private static final Quantity QUANTITY = new Length();
    private static final String NAME = "kilometer";
    private static final String ABBREV = "km";
    private static final String SYMBOL = "n/a";
    private static final String DESCRIPTION = "n/a";
    private static final UnitOfMeasure SIEQUIV = new Meter();
    private static final double SICONSTANT = 1000;
    
    public String getDescription() {
        return DESCRIPTION;
    }
    
    public String getName() {
        return NAME;
    }
    
    public Quantity getQuantity() {
        return QUANTITY;
    }
    
    public double getSIConstant() {
        return SICONSTANT;
    }
    
    public StandardOfMeasure getStandardOfMeasure() {
        return STANDARD;
    }
    
    public String getAbbrev() {
        return ABBREV;
    }
    
    public String getSymbol() {
        return SYMBOL;
    }
    
    public UnitOfMeasure getSIEquiv() {
        return SIEQUIV;
    }
    
    public String toString() {
        return NAME;
    }
    
 // multiple constant to convert to SI equivalent
    
}
